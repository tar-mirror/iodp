...
ifind(strhash_t *lv0,uint32 keylen, const char *key, unsigned int *ind,
	unsigned int *ind0)
{
	uint32 h;
	uint32 h0;
...
void strhash_lookupstart(strhash_t *lv0) { lv0->loop=0; }

int 
strhash_lookup(strhash_t *lv0,const char *key, uint32 keylen,
...
int strhash_lookupnext(strhash_t *lv0,const char *key, uint32 keylen,
    char **data, uint32 *datalen)
{
